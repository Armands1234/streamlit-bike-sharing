...
for column in datetime_columns:
    day_df[column] = pd.to_datetime(days_df[column])
    hour_df[column] = pd.to_datetime(hours_df[column])
...
main_df_day = days_df[(day_df["dteday"] >= str(start_date)) & 
                       (day_df["dteday"] <= str(end_date))]

main_df_hour = hours_df[(hour_df["dteday"] >= str(start_date)) & 
...
sns.barplot(x="hour", y="count_cr", data=sum_order_items_df.sort_values(by="hour", ascending=True).head(5), palette=["#D3D3D3", "#D3D3D3", "#D3D3D3", "#D3D3D3","#90CAF9"], ax=ax[1])
